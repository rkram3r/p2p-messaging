...
export const createConnection = (name, serverAddress) => async dispatch => {
  ...
  leftBuddy.on("signal", data =>
    socket.emit("p2p-connect", { data, name, id })
  );
  rightBuddy.on("signal", data =>
    socket.emit("p2p-connect", { data, name, id })
  );
};
...
export const createPeerConnection = (contactlist, idFrom, id) => dispatch => {
  ...
  const reciever = Array.from(contactlist)
    ...
    .map(([, { peer }]) => peer);
  const sendPeerConnection = (data, peer) =>
    peer.send(
      JSON.stringify({
        type: "PING",
        from: id,
...
export const send = ({ contactlist, sendTo }, message, from) => dispatch => {
  const selectedReciever =
    sendTo.length !== 0
      ? JSON.parse(sendTo)
          .filter(x => contactlist.get(x))
          .map(x => contactlist.get(x))
      : contactlist;
  const reciever =
    selectedReciever.length === 0 ? contactlist : selectedReciever;
