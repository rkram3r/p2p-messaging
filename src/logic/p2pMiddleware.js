...
export const sendContactlistToBuddy = store => next => action => {
  const {
    p2pReducer: { contactlist, id }
  } = store.getState();
  const { type, ...rest } = action;
  if (type === "PEER_READY") {
    const contactlistWithoutPeers = Array.from(contactlist).map(
      ([key, { peer, ...x }]) => [key, { ...x, state: "NOT_CONNECTED" }]
    );
    action.peer.send(
      JSON.stringify({
        type: "CONTACTLIST",
        contactlist: contactlistWithoutPeers
      })
    );
    Array.from(contactlist).forEach(([key, { peer, buddy }]) => {
      const filteredList = new Map([
        ...contactlistWithoutPeers.filter(([x]) => x !== key),
...
export const finalizeConnection = store => next => action => {
  ...
  const { type, from, to, data } = action;
  const peerFrom = contactlist.get(from) || false;
  if (type !== "PING") {
    return next(action);
  }
  if (peerFrom && peerFrom.state === "CONNECTING_PEER" && to && id === to) {
    const { peer, name } = contactlist.get(from);
    peer.on("data", msg =>
      store.dispatch({ ...JSON.parse(msg), id: from, name })
    );
    peer.on("connect", () =>
      store.dispatch({ type: "SET_PEER_READY", peer, key: from })
    );
    peer.signal(data);
  }
...
export const askToConnect = store => next => action => {
  ...
  const { type, from, to, ...rest } = action;
...
export const verify = store => next => action => {
  const { message, type, messageId, from } = action;
...
export const forwardPing = store => next => action => {
  ...
  const { type, from, to, lastPeer, ...rest } = action;
  ...
  if (type === "PING") {
    ...
    if (peerTo && peerTo.state !== "CONNECTING_PEER" && to && id !== to) {
      const reciever = Array.from(contactlist)
        .filter(
          ([key, { state, buddy }]) =>
            state === "PEER_READY" && lastPeer !== key && buddy
        )
        .map(([, { peer }]) => peer);
      const sendPeerConnection = peer =>
        peer.send(
          JSON.stringify({
            type: "PING",
            ...rest,
